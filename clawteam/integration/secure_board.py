...
class SecureBoardHandler(BoardHandler):
    ...
    def do_POST(self):
        ...
        path = urlparse(self.path).path
        if path == '/api/routing' or path.startswith(('/api/runtime/','/api/knowledge/','/api/workspaces','/api/task/','/api/budget/','/api/providers/','/api/folders/','/api/attachments/','/api/access/','/api/plans/', '/api/files/','/api/drafts/')):
