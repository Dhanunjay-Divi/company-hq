...
Read README.md, STATUS.md, ARCHITECTURE.md, UPSTREAM-REVIEW.md and REVIEWER.md first. Historical root guides describe the original installation and must not be interpreted as permission to modify it. For the current user-requested implementation sequence, use separate PRs for portable setup, onboarding, worker communication/visibility, and usage controls. No prior reviewer approval is required to implement or push a branch. Fetch before pushing, reconcile concurrent changes without force-pushing, and continue through the agreed milestones. The original reviewer inspects each bounded checkpoint and may make follow-up corrections. Changing the user's running installation remains a separate deployment action.
