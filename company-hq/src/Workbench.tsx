...
import { Layers3, Play, Network, LayoutGrid, Brain, Settings2, X, Menu, Scale, BookOpenCheck, Terminal, GitBranch, ShieldCheck, Monitor, Command, PanelRightOpen } from 'lucide-react';
...
function starterPrompt(goal: unknown) {
  const text = String(goal || '').trim();
  return `Inspect this project and create a concise read-only plan for: ${text || 'the outcome I describe'}. Ask only material questions. Propose the smallest useful team, acceptance checks, risks, budget limits and first implementation slice. Do not edit files yet.`;
}
...
export default function Workbench() {
  ...
  const capabilityTotal = Object.keys(capabilities).length;
  const capabilityReady = Object.values(capabilities).filter((item: any) => item?.available).length;
  ...
  const lastEvent = nonDeltaEvents[nonDeltaEvents.length - 1];
  const budget = runtime.budget || {};
  const budgetLabel = budget.limitTokens === 0 ? 'Budget off'
    : typeof budget.remainingTokens === 'number' ? `${budget.remainingTokens.toLocaleString()} tokens left`
    : 'Budget waiting for usage';
  const runtimeLabel = demo ? 'Demo · no model calls'
    : runtime.mode === 'execute' ? 'Execution approved'
    : runtime.connected ? 'Read-only planning'
    : 'Plan first';
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (dialog === 'workspace') {
      ...
      if (result) {
        await refreshList();
        setTeam(result.team);
        setDrafts(old => ({...old, [result.team]: starterPrompt(data.goal)}));
        setNotice('Starter message drafted. Press Discuss & plan when you want Codex to inspect read-only.');
        chooseView('run');
      }
  ...
  return <div className="desktop-frame">
    <header className="window-chrome" aria-label="Company HQ application shell">
      <div className="traffic-lights" aria-hidden="true"><span/><span/><span/></div>
      <div className="window-command"><Command size={14}/><span>Company HQ</span><kbd>⌘K</kbd></div>
      <div className="window-meta"><span>{capabilityReady}/{capabilityTotal || 0} capabilities</span><span>{budgetLabel}</span></div>
    </header>
    ...
    <aside className={`sidebar ${mobileNav ? 'is-open' : ''}`}>
      ...
      <div className="ide-mode-card"><span><Monitor size={14}/> Local project chat</span><span><GitBranch size={14}/> Approval before edits</span></div>
      ...
      {!team && <button className="sidebar-start" onClick={() => setDialog('workspace')}>New project chat</button>}
      <nav>{[['run',Play,'Chat'],['guide',BookOpenCheck,'How to use'],['map',Network,'Company map'],['board',LayoutGrid,'Work board'],['decisions',Scale,'Why this stack'],['memory',Brain,'Shared memory'],['system',Settings2,'System status']].map(([id,Icon,label]:any) => <button key={id} className={view === id ? 'active' : ''} onClick={() => chooseView(id)}><Icon size={17}/>{label}</button>)}</nav>
      {team ? <><div className="section-label">REGISTERED TEAM · {members.length}</div>
      ...
      : <><div className="section-label">START HERE</div><div className="sidebar-empty-note">Connect a project folder. Then the supervisor can plan before any writes.</div></>}
      {team && <button className="new-workspace" onClick={() => setDialog('workspace')}>+ Connect project</button>}
      <button className="small-button" onClick={() => setDialog('help')}>How this works</button>
    </aside>
    <main className="main-shell">
      <header className="topbar"><button className="icon-btn mobile-menu" aria-label="Open navigation" onClick={() => setMobileNav(true)}><Menu/></button><div className="breadcrumb"><span>Workspace</span><b>{profile.projectLabel || (team ? 'Company HQ' : 'No project yet')}</b></div>{team && <button className="command-chip" onClick={() => setConsoleOpen(true)}><Terminal size={14}/> Supervisor console</button>}<div className="top-controls"><span className={`runtime-pill ${runtime.state || 'offline'}`}><i/>{runtimeLabel}</span><span className="capability-pill"><ShieldCheck size={13}/>{capabilityReady}/{capabilityTotal || 0}</span>{team ? <button className="small-button" onClick={() => setDialog('task')}>Add task</button> : <button className="primary-button" onClick={() => setDialog('workspace')}>New chat</button>}</div></header>
      <section className="mission-bar"><div className="mission-icon"><Layers3 size={18}/></div><div><small>THE OUTCOME</small><h1>{profile.goal || 'Start a project chat and describe the result you need.'}</h1></div>{team ? <div className="mission-stats"><span><strong>{completedTasks}<em>/{tasks.length}</em></strong><small>tasks done</small></span><span><strong>{activeTasks}</strong><small>active</small></span><span><strong className={blockedTasks ? 'amber' : ''}>{blockedTasks}</strong><small>blocked</small></span></div> : <div className="mission-start"><small>No model calls until you send a prompt.</small></div>}</section>
