...
export default function Workbench() {
  ...
  return <div className="desktop-frame">
    ...
    <main className="main-shell">
      <header className="topbar"><button className="icon-btn mobile-menu" aria-label="Open navigation" onClick={() => setMobileNav(true)}><Menu/></button><div className="breadcrumb"><span>Workspace</span><b>{profile.projectLabel || 'Company HQ'}</b></div><button className="command-chip" onClick={() => setConsoleOpen(true)}><Terminal size={14}/> Supervisor console</button><div className="top-controls"><span className={`runtime-pill ${runtime.state || 'offline'}`}><i/>{runtimeLabel}</span><span className="capability-pill"><ShieldCheck size={13}/>{capabilityReady}/{capabilityTotal || 0}</span><button className="small-button" onClick={() => setDialog('task')} disabled={!team}>Add task</button></div></header>
      <section className="mission-bar"><div className="mission-icon"><Layers3 size={18}/></div><div><small>THE OUTCOME</small><h1>{profile.goal || 'Connect your project and describe the result you need.'}</h1></div><div className="mission-stats"><span><strong>{completedTasks}<em>/{tasks.length}</em></strong><small>tasks done</small></span><span><strong>{activeTasks}</strong><small>active</small></span><span><strong className={blockedTasks ? 'amber' : ''}>{blockedTasks}</strong><small>blocked</small></span></div></section>
      ...
      <div className={`work-area ${consoleOpen ? 'with-inspector' : ''}`}>
        <section className={`stage ${view === 'map' ? 'map-stage' : ''}`}>
          {view === 'run' && <RunOverview runtime={runtime} events={events as any} demo={demo} connectedProject={!!team} registeredCount={members.length} completedCount={completedTasks} taskCount={tasks.length} onConsole={() => setConsoleOpen(true)} onSystem={() => chooseView('system')} onBudget={() => setDialog('budget')} onDraft={text => setDrafts(old => ({...old,[team]:text}))}/>}
    ...
    <footer className="ide-statusbar" aria-label="Workspace status">
      ...
      <span><PanelRightOpen size={12}/>{consoleOpen ? 'Evidence dock open' : 'Evidence dock closed'}</span>
