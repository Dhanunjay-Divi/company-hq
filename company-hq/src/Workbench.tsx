...
export default function Workbench() {
  ...
  async function send() {
    ...
    try {
      ...
      await request(`/api/runtime/${encodeURIComponent(target)}/${status.connected ? 'send' : 'start'}`, {prompt, model: status.model || requestedModel, workMode, attachmentIds});
  ...
  return <div className="desktop-frame chat-app">
    ...
    <aside className={`sidebar ${mobileNav ? 'is-open' : ''}`}>
      ...
      <nav aria-label="Workspace tools">
        {([['run',MessageCircle,'Chat'],['map',Network,'Team'],['board',LayoutGrid,'Tasks'],['activity',Activity,'Activity'],['provider-tasks',Layers3,'Provider tasks']] as const).map(([id,Icon,label]) => <button key={id} className={view === id ? 'active' : ''} disabled={!team && id !== 'run' && id !== 'provider-tasks'} onClick={() => chooseView(id)}><Icon size={17}/>{label}{id==='board' && tasks.length>0 && <small>{tasks.length}</small>}</button>)}
      </nav>
