...
export default function Connections({ health, runtime, events = [], onRefresh, onModels }: Props) {
  ...
  React.useEffect(() => { loadProviders(true).catch((e: Error) => setError(e.message)); }, [loadProviders]); React.useEffect(() => { const dialog=dialogRef.current; if (selected && dialog && !dialog.open) dialog.showModal(); else if (!selected && dialog?.open) dialog.close(); }, [Boolean(selected)]);
  ...
  React.useEffect(()=>{if(selected?.authentication!=='signing_in')return;let active=true,pending=false;const providerId=selected.id;const timer=setInterval(async()=>{if(pending)return;pending=true;try{const response=await fetch('/api/providers');const value=await response.json();let row:Provider|undefined=value.providers?.find((p:Provider)=>p.id===providerId);if(active&&row){if(row.authentication==='not_checked'&&nativeProviders.has(providerId)){const check=await fetch(`/api/providers/${providerId}/check`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});const result=await check.json().catch(()=>({}));if(check.ok)row={...row,...result};}if(active&&row){setSelected(current=>current?.id===providerId?row!:current);setProviders(current=>current.map(p=>p.id===providerId?row!:p));}}}catch{}finally{pending=false}},1500);return()=>{active=false;clearInterval(timer)}},[selected?.id,selected?.authentication]);
